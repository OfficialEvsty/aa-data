...
func (r *ChainRepository) GetChain(ctx context.Context, nicknameID uuid.UUID) ([]*domain.NicknameChain, error) {
	...
	query := `SELECT chain_id, parent_chain_id, nickname_id, chained_at, active
			  FROM chains 
			  WHERE nickname_id = $1 
...
func (r *ChainRepository) GetRootChain(ctx context.Context, chainID uuid.UUID) (*domain.NicknameChain, error) {
	...
	query := `WITH RECURSIVE root_chain AS (
				...
				SELECT c.chain_id, c.nickname_id, c.parent_chain_id, c.chained_at, c.active
				FROM chains c
				INNER JOIN root_chain rc ON rc.parent_chain_id = c.chain_id
			)
			SELECT chain_id, nickname_id, parent_chain_id, active, chained_at
			FROM root_chain
			WHERE parent_chain_id IS NULL; -- находим корень`
	err := r.exec.QueryRowContext(ctx, query, chainID).Scan(&result.ChainID, &result.NicknameID, &result.ParentChainID, &result.Active, &result.ChainedAt)
	return &result, err
}
...
func (r *ChainRepository) GetActiveChainID(ctx context.Context, chainID uuid.UUID) (*domain.NicknameChain, error) {
	...
	query := `WITH RECURSIVE chain_tree AS (
		...
		SELECT DISTINCT chain_id, nickname_id, parent_chain_id, chained_at, active
		FROM chain_tree
		WHERE active = TRUE
		LIMIT 1;`
	err := r.exec.QueryRowContext(ctx, query, chainID).Scan(&result.ChainID, &result.NicknameID, &result.ParentChainID, &result.ChainedAt, &result.Active)
	return &result, err
}
